...
@Slf4j
public class GeneratedInternshipCertificateRepository extends NodeRepository {

    public static final String WORKSPACE = "internshipCertificates";
    public static final String NODE_TYPE = "mgnl:internshipCertificate";
    ...
    public GeneratedInternshipCertificateRepository() {
        this(WORKSPACE, NODE_TYPE);
    }

    public GeneratedInternshipCertificateRepository(String workspace, String nodeType) {
    ...
    public List<Node> findAllNodeByCode(String code) {
        try {
            ...
            Query query = qm.createQuery("SELECT * FROM [nt:base] AS t "
                    + "WHERE ([jcr:primaryType] = 'mgnl:internshipCertificate') "
    ...
    public Optional<Node> findByEnrolProgram(String id) {
        try {
            Query query = buildQuery(workspace, List.of(nodeType), Map.of("internship", List.of(id)));
            NodeIterator iterator = query.execute().getNodes();
            if (iterator.hasNext()) {
