...
import vn.ekino.certificate.dto.GeneratedInternshipCertificateDto;
import vn.ekino.certificate.dto.InternProgramDto;
...
@Singleton
@Slf4j
public class GeneratedInternCertificateService {
    //region Variables
    ...
    GeneratedInternshipCertificateDto dto;
    UserDto user;
    InternProgramDto internProgramDto;
    String imageId;
    String path;
    //endregion
    ...
    //region Constructor
    @Inject
    public GeneratedInternCertificateService(DamRepository damRepository, WebsiteRepository websiteRepository, DamTemplatingFunctions damFunctions, CertificateServicesModule certificateServicesModule, PublishingService publishingService) {
    ...
    //region Execute generate internship certificate files, page and publish
    ...
    private void generateCertificate(Node node) throws RepositoryException {
        dto = MapperUtils.nodeToObject(node, GeneratedInternshipCertificateDto.class).get();
        user = dto.getInternship().getUser();
        internProgramDto = dto.getInternship().getInternProgram();
        path = PropertyUtil.getString(node, GeneratedInternshipCertificateRepository.PROPERTY_PATH,"");
        ...
        String nodeName = String.format("%s-%s", internProgramDto.getNodeName(), user.getNodeName());
        PropertyUtil.setProperty(node, AssetNodeTypes.Asset.ASSET_NAME, nodeName);
        PropertyUtil.setProperty(node, GeneratedInternshipCertificateRepository.PROPERTY_PATH, path);
        PropertyUtil.setProperty(node, GeneratedInternshipCertificateRepository.PROPERTY_GENERATED_FILES, generatedFiles);
        PropertyUtil.setProperty(node, GeneratedInternshipCertificateRepository.PROPERTY_FULL_PAGE_PATH, String.format("%s/%s", certificateServicesModule.getServerPath(), path));
        setNodeName(node);
    ...
    private String createCertificatePage(String imageId) {
        try {
            ...
            PropertyUtil.setProperty(siteNode, WebsiteRepository.PROPERTY_TITLE, internProgramDto.getNodeName());
    ...
    private List<String> createCertificate() {
        ...
        try {
            ...
            //add text to image
            String fullName = dto.getInternship().getUser().getFullName();
            String startDate = formatDate(internProgramDto.getStartDate());
            String endDate = formatDate(internProgramDto.getEndDate());
            ...
            int x = 317;
            if (dto.getCode() != null){
                lengthOfCode = dto.getCode().length();
                if (6 < lengthOfCode && lengthOfCode <= 8) {
                    x = 277;
                }
                if (lengthOfCode > 8){
                    x = 237;
            ...
            drawString(imageTemplate, fullName, new Font(FONT_NAME, Font.BOLD, 100), graphics2D, 0, 1280);
            drawString(imageTemplate, startDate, new Font(FONT_NAME, Font.PLAIN, 47), graphics2D, 1282, 1569);
            drawString(imageTemplate, endDate, new Font(FONT_NAME, Font.PLAIN, 47), graphics2D, 1872, 1569);
            drawString(imageTemplate, domain + getPath(), new Font(FONT_NAME, Font.PLAIN, 47), graphics2D, x, 2180);
            graphics2D.drawImage(imageQR,551, 1843, null);
            graphics2D.dispose();
    ...
    private InputStream getCertificateTemplate() {
        String itemKey = "jcr:" + internProgramDto.getCertificateTemplateImage().getUuid();
